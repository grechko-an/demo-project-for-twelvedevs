...
- Name: e2e\product-catalog.spec.ts >> SauceDemo Product Catalog & Sorting Tests >> Accessibility and UI Tests >> Verify product card accessibility
- Location: tests\e2e\product-catalog.spec.ts:259:9

# Error details

```
Error: locator.getAttribute: Error: strict mode violation: locator('.inventory_item').first().locator('.inventory_item_img') resolved to 2 elements:
    1) <div class="inventory_item_img">…</div> aka locator('.inventory_item_img').first()
    2) <img alt="Sauce Labs Backpack" class="inventory_item_img" data-test="inventory-item-sauce-labs-backpack-img" src="/static/media/sauce-backpack-1200x1500.0a0b85a385945026062b.jpg"/> aka locator('[data-test="item-4-img-link"]')
